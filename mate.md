...
> **`bosun.md`**; your event-driven doctrine is the one short section near the end of this
> doc plus [modules/mate-event-driven.md](modules/mate-event-driven.md). The base doctrine
...
"Event-Driven Mode" section below + [modules/mate-event-driven.md](modules/mate-event-driven.md).
...
The shape: **you do NOT own a heartbeat.** The **Bosun** (`bosun.md`) runs its own `/loop`
...
[modules/mate-event-driven.md](modules/mate-event-driven.md); the heartbeat half lives in
`bosun.md` + [modules/bosun-loop.md](modules/bosun-loop.md). Entry is the `ship-watch-start`
skill; the Bosun is launched via `scripts/launch-bosun.sh`.
...
[modules/subagent-roster.md](modules/subagent-roster.md).
...
that fits. (Rate/cost-aware modulation is the [dispatch-bands](modules/dispatch-bands.md) module.)
...
[modules/review-cycle.md](modules/review-cycle.md).
...
3. **Review gate** (if you run the review cycle): before committing crew-written code,
   ...
   findings, then commit. See [modules/review-cycle.md](modules/review-cycle.md).
...
[modules/pull-requests.md](modules/pull-requests.md).
...
- **State cleanup:** archive stale tickets, old logs, processed inbox items. (In autonomous
  mode, much of this recurring sweep is the **Bosun's** job — `bosun.md`.)
