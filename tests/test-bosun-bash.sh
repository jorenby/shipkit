...
# Usage: ./tests/test-bosun-bash.sh

set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
HOOK="$SCRIPT_DIR/../scripts/validate-bosun-bash.sh"
PASS=0
FAIL=0
...
check 0 ship-bosun 'python3 scripts/bosun_emit.py heartbeat alive'
check 0 ship-bosun 'python3 scripts/bosun_emit.py drop t f a'
