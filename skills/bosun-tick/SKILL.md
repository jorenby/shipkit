...
1. **Heartbeat (always, first).** `python3 scripts/bosun_emit.py heartbeat "<one-line
...
5. **Classify findings → WAKE vs SILENT, and act.**
   - **WAKE** — a state change on a tracked PR, a maintainer comment proposing a direction
     ...
     → write ONE drop: `python3 scripts/bosun_emit.py drop "<title>" "<findings md>"
...
6. **Update the cursor + self-pace.**
   - `python3 scripts/bosun_emit.py cursor '<json of this sweep's state>'` so the next
