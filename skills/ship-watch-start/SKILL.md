...
All paths are relative to the ship root. The *meaning* of each step lives in `mate.md`
(and `modules/mate-event-driven.md`) — read it there, don't re-derive.
...
`ruby scripts/mate-lock.rb acquire bg-mate-$(date +%s)` (or `python3 scripts/mate-lock.py
acquire …`), then `status`. A prior bg-Mate's lock is usually STALE (an event-driven Mate
...
- Run `python3 scripts/wake_monitor.py` under the Monitor tool. It watches `inbox/drops/`
  + `inbox/captain.md`, classifies each net-new item through `classify_input.py`, and
  emits one `WAKE <reason>` line per wake-class item (the Monitor turns each into a wake).
  See `modules/wake-monitor.md` for the contract + the dedup/zsh-nomatch/content-hash
...
already ticking:
- `scripts/launch-bosun.sh --ensure` — launches a bg Bosun ONLY if
