...
shipkit ships a working wake-monitor: **`scripts/wake_monitor.py`** — the companion this
...
- Reuses `scripts/classify_input.py` so a declared `wake_class` is honored verbatim (no
...
Tests: `tests/test_wake_monitor.py` covers wake-steer fires, batch silent, dedup across a
...
filesystem-watch variant: **`scripts/wake_monitor_native.py`** — behaviorally identical
