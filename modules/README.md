...
| [night-economy/night-economy.md](night-economy/night-economy.md) | 2 | Autonomous mode | Watch cadence + overnight posture: fresh daily rotation, economy-model night Mate with a standing self-escalation license, the read-only night watch (no external writes; local-only dispatch). Doc-only. |
...
| [navigator/navigator.md](navigator/navigator.md) | 1 | Ships wanting a strategy seat | A second operating seat that **advises and does not execute**: researches, shapes tickets, reviews architecture, owns queue *strategy* — but never writes `queue.md`, dispatches, or commits product code. Its queue judgment arrives as drops the Mate applies, so the single-writer rule holds. Also carries the Captain-facing communication rules (restated from core, where they are the parent) and the drop-pickup protocol a loopless seat needs. Opt-in (`--modules navigator`), in no preset — a distinct seat shouldn't switch itself on; core is a complete ship without it. |
| [pilot/pilot.md](pilot/pilot.md) | 1 | Browser watches | **Ships the `ship-pilot` agent def** (`agents/ship-pilot.md`) — standard crew tools + git-safety hook *plus* the `claude-in-chrome` MCP tools. Hard external Chrome-MCP dependency, so opt-in (`--modules pilot`), not in any preset; dispatch only when the Captain authorizes browser access. Reuses core's crew Bash hook + Write/Edit path guard (no hook of its own). |
