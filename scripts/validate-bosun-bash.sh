...
# `scripts/bosun_emit.py` (its path-locked write helper, which can only touch
...
if echo "$COMMAND" | grep -qE '(^|[[:space:]|;&])(tee|dd)([[:space:]]|$)'; then
  echo "Blocked: Bosun writes ONLY via scripts/bosun_emit.py — no tee/dd." >&2
  exit 2
fi
...
if echo "$SAN_REDIR" | grep -qE '>'; then
  echo "Blocked: Bosun cannot redirect output to files — use scripts/bosun_emit.py (heartbeat|cursor|drop)." >&2
  exit 2
fi
...
check_allowed() {
  ...
  # Source scripts/bosun-allow-local.sh to add deployment-specific read-only tools
