...
HOOKS=(validate-mate-bash.sh validate-mate-mcp.sh validate-bosun-bash.sh validate-crew-bash.sh)
...
preflight() {
  ...
  # 3. Lock cycle works.
  echo "[lock]"
  if command -v ruby >/dev/null 2>&1 && [ -f "$SHIP/scripts/mate-lock.rb" ]; then
    LOCKER="ruby scripts/mate-lock.rb"
  else
    LOCKER="python3 scripts/mate-lock.py"
  fi
...
case "$MODE" in
  --check)
    ...
    "$SHIP/scripts/launch-bosun.sh" --check 2>/dev/null | sed 's/^/  /'
  ...
  --rotate-mate)
    ...
    LOCKER=$(command -v ruby >/dev/null 2>&1 && [ -f "$SHIP/scripts/mate-lock.rb" ] && echo "ruby scripts/mate-lock.rb" || echo "python3 scripts/mate-lock.py")
